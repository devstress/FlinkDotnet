# Flink.NET Samples - Real Apache Flink 2.1.0 Integration

This directory contains samples demonstrating Flink.NET's **real Apache Flink 2.1.0 integration** for production Kubernetes deployment.

## 🏗️ Architecture Overview

The samples demonstrate the new approach using **real Flink 2.1.0 execution**:
- **Real Job Submission** - Actual job submission to live Flink 2.1.0 clusters via REST API
- **Kubernetes Integration** - Production-ready deployment patterns with K8s service discovery
- **No Simulation** - All jobs execute on actual Apache Flink 2.1.0 JobManager and TaskManager instances

```
┌─────────────────────┐   ┌──────────────────────┐   ┌─────────────────────┐
│  .NET Application   │──▶│  Flink Job Gateway   │──▶│  Apache Flink 2.1.0   │
...
**Real Flink 2.1.0 Integration Reliability Test** - BDD-style tests with actual job execution

**Features:**
- **Real Job Submission** - Submits actual jobs to live Flink 2.1.0 cluster
...
**Test Coverage:**
- Real job submission and execution via Flink Job Gateway to Flink 2.1.0 cluster
- JSON Intermediate Representation (IR) translation to actual Flink DataStream jobs
- Job Gateway communication with real Flink 2.1.0 JobManager and TaskManager
...
**Basic Streaming Examples with Real Flink 2.1.0 Integration**

**Features:**
- **Kubernetes Service Discovery** - Locates Flink Job Gateway via K8s service names
- **Real Job Execution** - All examples execute on actual Flink 2.1.0 cluster
...
### Prerequisites
- .NET 9.0 SDK
- Apache Flink 2.1.0 cluster (JobManager + TaskManager)
...
### Real Flink 2.1.0 Integration

All samples integrate with actual Apache Flink 2.1.0 clusters:

```csharp
// Real job submission to Flink 2.1.0 cluster
...
- **Infrastructure validation** - Real Flink 2.1.0 cluster connectivity
...
When connected to actual Flink 2.1.0 cluster:
...
### Production Metrics
- **Real job execution metrics** from Flink 2.1.0 cluster
...
```yaml
- name: Deploy Flink 2.1.0 Cluster
  run: |
    kubectl apply -f k8s/
...
1. **Deploy** - Real Flink 2.1.0 cluster and Job Gateway
...
- **k8s/README.md** - Kubernetes deployment guide for Flink 2.1.0

### API Documentation
- **Flink Job Gateway** - Real REST API documentation
- **Apache Flink 2.1.0** - Official Flink documentation and monitoring
...
1. **Use real Flink 2.1.0 integration** - No simulation or mocking
...
- [Apache Flink 2.1.0 Documentation](https://flink.apache.org/docs/)
...
**All samples demonstrate real Apache Flink 2.1.0 integration with actual job execution and production-ready Kubernetes deployment patterns.**