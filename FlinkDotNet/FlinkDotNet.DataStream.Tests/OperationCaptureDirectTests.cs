...
namespace FlinkDotNet.DataStream.Tests
{
    ...
    public class OperationCaptureDirectTests
    {
        ...
        public void ToJobDefinition_WithSerializerFunction_AddsMetadata()
        {
            // Arrange
            var stream = this._env.FromKafka("input-topic", "localhost:9092", "test-group", "earliest");
            _ = stream.SinkToKafka("output-topic", "localhost:9092", s => s);
...
#pragma warning disable S4144 // Methods should not have identical implementations
        ...
        public void TranslateAggregateOperation_WithTimeBasedWindow_ConvertsToSeconds()
        {
            ...
            _ = result.SinkToKafka("output-topic", "localhost:9092", x => x.ToString());
        ...
        {
            // Test with "latest"
            var stream1 = this._env.FromKafka("test-topic", "localhost:9092", "test-group", "latest");
            Assert.That(stream1, Is.Not.Null);

            // Test with "earliest"
            var stream2 = this._env.FromKafka("test-topic", "localhost:9092", "test-group", "earliest");
