...
public class GlobalTestInfrastructure
{
    ...
    public async Task GlobalSetUp()
    {
        ...
        try
        {
            ...
            IDistributedApplicationTestingBuilder appHost = await DistributedApplicationTestingBuilder.CreateAsync<Projects.LocalTesting_FlinkSqlAppHost>();
            ...
            for (int attempt = 1; attempt <= 10; attempt++) // 10 attempts × 3s = 30s max
            {
                ...
                string containers = await RunDockerCommandAsync("ps --filter name=kafka --format \"{{.Names}}\"");
                if (!string.IsNullOrWhiteSpace(containers))
                {
                    ...
                    string allContainers = await RunDockerCommandAsync("ps --format \"table {{.Names}}\\t{{.Status}}\\t{{.Ports}}\"");
            ...
            string kafkaContainerIp = await GetKafkaContainerIpAsync();
            ...
            string discoveredKafkaEndpoint = await GetKafkaEndpointAsync();
            ...
            string gatewayEndpoint = await GetGatewayEndpointAsync();
        ...
        catch (Exception ex)
        {
            ...
            string diagnostics = await GetContainerDiagnosticsAsync();
    ...
    private static void CleanupTestLogsDirectory()
    {
        try
        {
            ...
            string testLogsDir = Path.Combine(repoRoot, "LocalTesting", "test-logs");
    ...
    private static async Task CaptureContainerLogAsync(string containerNameFilter, string outputPath)
    {
        try
        {
            ...
            string containerList = await RunDockerCommandAsync($"ps -a --filter \"name={containerNameFilter}\" --format \"{{{{.Names}}}}\"");
            List<string> containers = containerList.Split('\n', StringSplitOptions.RemoveEmptyEntries)
                .Select(c => c.Trim())
            ...
            string logCheck = await RunDockerCommandAsync($"logs {containerName} --tail 1 2>&1");
            ...
            string logs = await RunDockerCommandAsync($"logs {containerName} 2>&1");
    ...
    private static void ConfigureGatewayJarPath()
    {
        ...
        string releaseJarPath17 = Path.Combine(repoRoot, "FlinkDotNet", "Flink.JobGateway", "bin", "Release", "net9.0", "flink-ir-runner-java17.jar");
        ...
        string debugJarPath17 = Path.Combine(repoRoot, "FlinkDotNet", "Flink.JobGateway", "bin", "Debug", "net9.0", "flink-ir-runner-java17.jar");
    ...
    private static async Task ValidateDockerEnvironmentAsync()
    {
        ...
        try
        {
            string dockerInfo = await RunDockerCommandAsync("info --format \"{{.ServerVersion}}\"");
    ...
    private static async Task LogTaskManagerStatusAsync()
    {
        try
        {
            ...
            string containerName = await RunDockerCommandAsync("ps --filter name=flink-taskmanager --format \"{{.Names}}\" | head -1");
            ...
            string status = await RunDockerCommandAsync($"ps --filter \"name={containerName}\" --format \"{{{{.Status}}}}\"");
            Console.WriteLine($"📊 Container status: {status.Trim()}");

            // Get last 100 lines of TaskManager logs
            string logs = await RunDockerCommandAsync($"logs {containerName} --tail 100");
    ...
    private static async Task<string> GetFlinkJobManagerEndpointAsync()
    {
        try
        {
            ...
            foreach (string line in lines)
            {
                ...
                {
                    System.Text.RegularExpressions.Match match = System.Text.RegularExpressions.Regex.Match(line, @"127\.0\.0\.1:(\d+)->8081");
                    if (match.Success)
                    {
    ...
    private static Task<string> GetGatewayEndpointAsync()
    {
        ...
        string gatewayEndpoint = $"http://localhost:{Ports.GatewayHostPort}/";
    ...
    private static async Task<string> GetTemporalEndpointAsync()
    {
        try
        {
            string temporalContainers = await RunDockerCommandAsync("ps --filter \"name=temporal-server\" --format \"{{.Ports}}\"");
            string[] lines = temporalContainers.Split('\n', StringSplitOptions.RemoveEmptyEntries);
            ...
            foreach (string line in lines)
            {
                ...
                {
                    System.Text.RegularExpressions.Match match = System.Text.RegularExpressions.Regex.Match(line, @"127\.0\.0\.1:(\d+)->7233");
                    if (match.Success)
                    {
    ...
    private static async Task<string> GetKafkaEndpointAsync()
    {
        try
        {
            string kafkaContainers = await RunDockerCommandAsync("ps --filter \"name=kafka\" --format \"{{.Ports}}\"");
    ...
    {
        string[] lines = kafkaContainers.Split('\n', StringSplitOptions.RemoveEmptyEntries);
        foreach (string line in lines)
        {
            ...
            System.Text.RegularExpressions.Match match = System.Text.RegularExpressions.Regex.Match(line, @"(?:127\.0\.0\.1|0\.0\.0\.0):(\d+)->9092");
            if (match.Success)
            {
    ...
    private static async Task<string> GetKafkaContainerIpAsync()
    {
        try
        {
            string kafkaContainers = await RunDockerCommandAsync("ps --filter \"name=kafka-\" --format \"{{.Names}}\"");
            string? kafkaContainer = kafkaContainers.Split('\n', StringSplitOptions.RemoveEmptyEntries).FirstOrDefault();
            ...
            string ipAddress = await RunDockerCommandAsync($"inspect {kafkaContainer} --format \"{{{{.NetworkSettings.Networks.bridge.IPAddress}}}}\"");
    ...
    private static async Task<string> GetContainerDiagnosticsAsync()
    {
        try
        {
            ...
            // Try Docker first
            string? dockerContainers = await TryRunContainerCommandAsync("docker", "ps -a --format \"table {{.Names}}\\t{{.Status}}\\t{{.Ports}}\"");
            ...
            string? podmanContainers = await TryRunContainerCommandAsync("podman", "ps -a --format \"table {{.Names}}\\t{{.Status}}\\t{{.Ports}}\"");
        ...
        {
            string errorMsg = $"⚠️ Failed to get container diagnostics: {ex.Message}";
