...
/// <summary>
/// Flink diagnostic and logging utilities for troubleshooting job failures.
/// </summary>
internal static class FlinkDiagnostics
{
/// <summary>
...
/// </summary>
public static async Task<string> GetFlinkJobManagerLogsAsync(string flinkEndpoint)
{
    try
    {
...
/// </summary>
public static async Task<string> GetFlinkJobExceptionsAsync(string flinkEndpoint, string jobId)
{
    try
    {
...
/// </summary>
public static async Task<string> GetFlinkTaskManagerLogsAsync(string flinkEndpoint)
{
    try
    {
        using var httpClient = new System.Net.Http.HttpClient { Timeout = TimeSpan.FromSeconds(10) };
...
/// </summary>
public static async Task<string> GetTaskManagerLogsFromDockerAsync()
{
    try
    {
        ...
        var logs = await DockerUtilities.RunDockerCommandAsync($"logs {containerName} --tail 20 2>&1");
...
/// </summary>
public static async Task<string> GetFlinkJobDiagnosticsAsync(string flinkEndpoint, string? jobId = null)
{
...
/// </summary>
public static async Task LogJobStatusViaGatewayAsync(string gatewayBase, string jobId, string checkpoint)
{
    try
    {
...
/// </summary>
public static async Task LogFlinkContainerStatusAsync(string checkpoint)
{
    try
    {
        ...
        // Get ALL container names and filter in C# to handle Aspire's random suffixes
        var allContainersList = await DockerUtilities.RunDockerCommandAsync("ps --format \"{{.Names}}\"");
        ...
        if (!string.IsNullOrWhiteSpace(jmName))
        {
            ...
            var jmLogs = await DockerUtilities.RunDockerCommandAsync($"logs {jmName} --tail 100 2>&1");
        ...
        if (!string.IsNullOrWhiteSpace(tmName))
        {
            ...
            var tmLogs = await DockerUtilities.RunDockerCommandAsync($"logs {tmName} --tail 20 2>&1");
...
/// </summary>
public static async Task LogFlinkJobLogsAsync(string jobId, string checkpoint)
{
    try
    {
        ...
        if (!string.IsNullOrWhiteSpace(jmName))
        {
            ...
            var jobLogs = await DockerUtilities.RunDockerCommandAsync($"logs {jmName} 2>&1");
        ...
        if (!string.IsNullOrWhiteSpace(tmName))
        {
            ...
            var allLogs = await DockerUtilities.RunDockerCommandAsync($"logs {tmName} 2>&1");
